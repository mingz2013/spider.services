...
def check_pushed(reg_bus_ent_id):
    ...
    if company and company.get(u"tzr_pushed") == 1:
        return True
    else:
        return False
...
def push_tzr_list(tzr_list, reg_bus_ent_id, company_name):
    if check_pushed(reg_bus_ent_id):
        logging.info("pass........")
        return

    # url = "https://data.api.zhironghao.com/update/qiyenianbao"
    url = "https://data.api.zhironghao.com/update/touziren"
    ...
    for tzr in d:
        ...
        for (k, v) in tzr.items():
            ...
            if k in info_templ.keys():
                ...
                logging.error("not have k in templ in tzr_info_templ  ...k:%s" % k)
                exit(-1)
        l.append(i)
